...
use esp_hal::{
    ...
    rmt::{Channel, ConfigError, Error as RmtError, PulseCode, Tx, TxChannelConfig, TxChannelCreator},
};
...
pub enum AdapterError {
    ...
    BufferSizeExceeded,
    /// Raised if something goes wrong during channel configuration.
    ...
    Timing: crate::Timing,
{
    ...
    pub fn new<Ch, P>(channel: Ch, pin: P) -> Result<Self, AdapterError>
    where
    ...
    ///
    /// If any configuration issue with the RMT [`Channel`] occurs, the error will be returned.
    pub fn new_with_memsize<Ch, P>(channel: Ch, pin: P, memsize: u8) -> Result<Self, AdapterError>
    where
        Ch: TxChannelCreator<'d, Mode>,
        P: PeripheralOutput<'d>,
    {
